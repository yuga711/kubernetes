...
// cs/piper///depot/google3/cloud/kubernetes/server/hostedmaster/hostedmasterserver.go?type=cs&rcl=284680803&l=1015
...
func NewGkeApiserverWebhookAuthorizer(kubeConfigFile string, version string, authorizedTTL, unauthorizedTTL time.Duration, customDial utilnet.DialFunc) (RuleResolverAuthorizer, error) {
	// Create regular WebhookAuthorizer
	webhookAuthorizer, err := New(kubeConfigFile, version, authorizedTTL, unauthorizedTTL, customDial)
	if err != nil {
		return nil, err
	}
